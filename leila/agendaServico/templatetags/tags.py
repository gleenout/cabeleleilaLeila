...
class ElementNode(template.Node):
    ...
    def render(self, context):
        ...
        template_names = []
        if layout:
            template_names.append(f"base_templates/elementos/{self.element}__{layout}.html")
            template_names.append(f"teste/elementos/{self.element}__{layout}.html")
        template_names.append(f"base_templates/elementos/{self.element}.html")
        template_names.append(f"teste/elementos/{self.element}.html")
...
def navactive(context, url_name, output=None):
    if not output:
        output = 'active'
    try:
...
def addclass(value, arg):
    ...
    """
    class_attr = value.field.widget.attrs.get('class', '')
    value.field.widget.attrs['class'] = f'{class_attr} {arg}'.strip()
    return value

@register.filter(name='addattr')
def addattr(value, arg):
    """
    Adiciona um atributo HTML a um campo de formulário.
    """
